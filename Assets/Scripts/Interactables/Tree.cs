...
/// <summary>
/// A tree interactable that periodically spawns sticks in an area around it.
...
public class Tree : Interactable
{
	...
	[Tooltip("Maximum distance from the tree that sticks can spawn (in grid tiles).")]
	[SerializeField] private int _maxSpawnRadius = 5;
	[Tooltip("Maximum number of sticks that can exist around this tree at once.")]
  ...
  public override string GetKnowledgeTitle() {
	...
	/// <summary>
	/// Initializes the tree at the specified grid position.
	/// </summary>
	...
	private bool TrySpawnSticksItem()
	{
		if (EnvironmentManager.Instance == null)
		{
			Debug.LogWarning("Tree: EnvironmentManager instance not found. Cannot spawn sticks.");
			return false;
		}

		if (ItemManager.Instance == null)
		{
			Debug.LogWarning("Tree: ItemManager instance not found. Cannot spawn sticks.");
			return false;
		}
		...
		{
			Debug.LogWarning($"Tree: No valid walkable positions found in spawn area around ({_gridPosition.x}, {_gridPosition.y}).");
			return false;
		}
		...
		foreach (Vector2Int spawnPos in validPositions)
		{
			...
			if (spawnedStick != null)
			{
				...
				Debug.Log($"Tree: Spawned sticks item at ({spawnPos.x}, {spawnPos.y}). Total sticks: {_spawnedSticks.Count}/{_maxSticks}");
				return true;
			}
		}
	...
	private List<Vector2Int> GetValidWalkablePositionsInArea()
	{
		...
		for (int dx = -maxRadius; dx <= maxRadius; dx++)
		{
			for (int dy = -maxRadius; dy <= maxRadius; dy++)
			{
				...
				// Calculate Manhattan distance from tree
